//! Data models for Merlin API requests and responses.
//!
//! This module contains all the data structures used for API communication,
//! including request/response types, validation helpers, and error types.
...
/// Result type for preference deletion operations.
...
/// Creates an error API response with the given error.
pub fn create_error_response(error: APIError) -> APIResponse<String> {
    APIResponse::error(error)
}

/// Creates a validation error with an optional field name.
pub fn create_validation_error(message: &str, field: Option<&str>) -> APIError {
    APIError::validation(message.to_string(), field.map(|s| s.to_string()))
}
...
pub fn create_conflict_error(message: &str) -> APIError {
    APIError::new(ErrorCode::ConflictError, message.to_string(), None)
}

/// Creates an internal server error with the given message.
pub fn create_internal_error(message: &str) -> APIError {
    APIError::new(ErrorCode::InternalServerError, message.to_string(), None)
}
...
/// Validates that max_tokens is between 1 and 8192.
...
/// Validates that the model name is a known valid model.
...
/// Validates that the preference category is valid.
...
/// Validates that the feedback category is valid.
...
/// Validates that a JSON preference value is not null or empty.
pub fn validate_preference_value(value: &serde_json::Value) -> Result<(), APIError> {
    ...
    if let Some(s) = value.as_str() {
        if s.trim().is_empty() {
            return Err(create_validation_error(
                "String preference value cannot be empty",
                Some("preference_value"),
            ));
        }
    }
    ...
    if let Some(arr) = value.as_array() {
        if arr.is_empty() {
            return Err(create_validation_error(
                "Array preference value cannot be empty",
                Some("preference_value"),
            ));
        }
    }
...
/// Returns the default max tokens value.
